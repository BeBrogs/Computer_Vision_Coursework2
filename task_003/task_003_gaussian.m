...
while count <= total_num_frames
    ...
    current_frame = imnoise(read(v_reader, count), "gaussian", 0.1);
    ...
    for i=1: len_of_index_arr
        ...
        gaussian_frame = double(imnoise(read(v_reader, frame_index_arr(i)), "gaussian", 0.1));
        

        %REMOVE GAUSSIAN NOISE
        this_img = remove_gaussian_noise(gaussian_frame);
    
        %THROUGH AVERAGE FILTER
        %this_img = remove_gaussian_noise_avg(gaussian_frame);
    ...
    %AVG FILTER
    %current_frame = remove_gaussian_noise_avg(current_frame);
...
function removed_gaussian = remove_gaussian_noise(frame)
    rgb_range = 3;
    for i=1: rgb_range
        removed_gaussian(:,:,i) = wiener2(frame(:,:,i), [4,4]);
    end
end

function removed_gaussian = remove_gaussian_noise_avg(frame)
    k = ones(4,4)/16;
    removed_gaussian = imfilter(frame, k);
